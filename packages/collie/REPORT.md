...
> | 5. 파이프라인 구조도 | 구조 설명(특정 코퍼스 아님) | 혼합. 완화 축 수치표는 현행이나 "retrieve 스텁" 서술은 옛 상태다 | `src/graph.ts`·`config/default.json` |
