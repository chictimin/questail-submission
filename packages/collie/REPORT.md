...
> **2026-09-21 릴리즈 정정:** 아래 4~6절은 중단된 개인 real 코퍼스 실험의 기록이며, 현재 공개 demo의 동작 명세가 아니다. 현재 공개 경로는 `questail collie serve --demo`가 합성 50건만 임시 생성해 검색하고, UI·CLI 모두 실제 `/ask` SSE를 호출해 경로·원문 근거만 보여준다. `respond` 생성, `check` 검증, core agent 도구 실행 및 개인 real 코퍼스 질의는 이번 공개 demo 릴리즈 범위에서 제외한다. 따라서 4~6절의 7노드 실행·웹 답변·serve/ask 동치 관련 주장은 현재 제품 주장으로 읽으면 안 된다.
